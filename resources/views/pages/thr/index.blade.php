  ...
  <div class="col-12">
    ...
    <div class="card">
      <div class="card-body">
            ...
            <tbody>
              <form action="{{ route('thr.store') }}" method="POST">
                ...
                <tr>
                  ...
                  <td class="align-items-center text-center text-nowrap p-1"><input style="min-width: 200px; width: 100%;" readonly type="text" class="form-control border border-danger" name="nominal" value="{{ $user->karyawan->nominal_tunjangan_hari_raya }}" required></td>
              ...
              @foreach($thrs as $thr)
                <form action="{{ route('thr.update', $thr->id) }}" method="POST">
                  ...
                  <tr>
                    <input type="hidden" class="form-control" name="karyawan_id" value="{{ $user->karyawan->id }}">
                    {{-- <td class="align-items-center text-center text-nowrap">{{ $thrs->firstItem() + $loop->index }}</td> --}}
                    <td class="align-items-center text-center text-nowrap">{{ $loop->iteration }}</td>
                    <td class="align-items-center text-center text-nowrap p-1"><input style="min-width: 200px; width: 100%;" type="month" class="form-control" name="tanggal" value="{{ $thr->tanggal }}"></td>
                    <td class="align-items-center text-center text-nowrap p-1"><input style="min-width: 200px; width: 100%;" readonly type="text" class="form-control @error('nominal') border border-danger @enderror" name="nominal" value="{{ $thr->nominal }}"></td>
                    <td class="align-items-center text-center text-nowrap p-1"><input style="min-width: 200px; width: 100%;" type="text" class="form-control @error('nominal_yang_dibayarkan') border border-danger @enderror" name="nominal_yang_dibayarkan" value="{{ $thr->nominal_yang_dibayarkan }}"></td>
                    <td class="align-items-center text-center text-nowrap p-1"><input style="min-width: 200px; width: 100%;" readonly type="text" class="form-control @error('sisa') border border-danger @enderror" name="sisa" value="{{ $thr->sisa }}"></td>
                    <td class="align-items-center text-nowrap">
                      @if(auth()->user()->hasPermission('thr-edit'))
                        <button type="submit" class="btn btn-icon btn-primary"><i class="fas fa-check"></i></button>
                      @endif
                      @if(auth()->user()->hasPermission('thr-delete'))
                        <a href="{{ route('thr.delete', $thr->id) }}" class="btn btn-icon btn-danger delete2"><i class="fas fa-trash"></i></a>
                      @endif
                    </td>
                  </tr>
                </form>
              ...
              <tr>
                ...
                <td class="align-items-center text-nowrap"></td>
                <td class="align-items-center text-center text-nowrap p-1"><input readonly type="text" class="form-control" name="" id="totalTHR"></td>
        ...
        {{-- <div class="float-right">
          {{ $thrs->links('pagination::bootstrap-4') }}
        </div> --}}
      </div>
    </div>
  </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const totalTHRInput = document.getElementById('totalTHR');
    ...
    function updateTotal() {
      ...
      nominalInputs.forEach((input, index) => {
        if (index > 0) {
          totalTHR += parseFloat(input.value.replace(/[^\d]/g, '') || 0);
        }
      });
      totalTHRInput.value = formatter.format(totalTHR).replace(',00', '');
